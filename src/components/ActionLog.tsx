  ...
  return (
    <div className="h-full flex flex-col">
      <div className="p-3 border-b border-game-border">
        <h3 className="text-xs font-semibold text-game-muted uppercase tracking-wider">News Feed</h3>
      </div>
      <div className="flex-1 overflow-y-auto p-3 space-y-2">
        {gameState.actionLog.slice().reverse().map((entry, i) => (
          <div key={i} className="text-xs animate-fade-in">
            <div className="flex items-center gap-2 mb-0.5">
              <span className="text-game-muted">T{entry.turn}</span>
              <span className={`font-medium ${typeColors[entry.type] ?? 'text-game-secondary'}`}>
                {entry.message}
              </span>
            </div>
          </div>
        ...
        {gameState.actionLog.length === 0 && (
          <div className="text-xs text-game-muted text-center mt-8">
            No events yet...
          </div>
        )}
      </div>
