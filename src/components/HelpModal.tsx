...
const slides = [
  {
    ...
    content: (
      <div className="space-y-4 text-sm text-white">
        ...
        <div className="bg-game-border/50 rounded-lg p-3">
...
export function HelpModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div className="bg-game-card border border-game-border rounded-xl max-w-md w-full max-h-[80vh] flex flex-col animate-fade-in">
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-game-border">
          ...
          <button
            onClick={onClose}
            className="text-game-secondary hover:text-white w-8 h-8 flex items-center justify-center rounded-lg hover:bg-game-border transition-all cursor-pointer"
          >
            ×
          </button>
        </div>
        ...
        {/* Footer */}
        <div className="flex items-center justify-between p-4 border-t border-game-border">
          {/* Dots */}
          <div className="flex gap-1.5">
            {slides.map((_, i) => (
              <button
                ...
                className={`w-2 h-2 rounded-full transition-all cursor-pointer ${i === slideIndex ? 'bg-blue-500 w-4' : 'bg-game-muted/30'}`}
              />
            ))}
          </div>
          ...
          <div className="flex gap-2">
            {slideIndex > 0 && (
              <button
                ...
                className="px-4 py-2 text-sm text-game-secondary hover:text-white transition-all cursor-pointer"
              >
                ← Back
              </button>
            )}
